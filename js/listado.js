...
Listado.prototype.obtenerCiudades = function() {
    let listadoDeCiudades = this.restaurantes.map(restaurante => restaurante.ubicacion);
    return filtrarElementosRepetidos(listadoDeCiudades).sort();
}
...
Listado.prototype.obtenerRubros = function() {
    let listadoDeRubros = this.restaurantes.map(restaurante => restaurante.rubro);
    return filtrarElementosRepetidos(listadoDeRubros).sort();
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    let horariosExistentes = this.restaurantes.map(restaurante => restaurante.horarios);
    //En este arreglo vamos a poner todos los horarios, uno por uno
    var listadoDeHorarios = [];
    horariosExistentes.map(horarios => horarios.map(horario => listadoDeHorarios.push(horario)));
    return filtrarElementosRepetidos(listadoDeHorarios).sort()
}
//filtroRubro null, filtroCiudad null, filtroHorario null
...
function filtrarElementosRepetidos(arregloElementos){
    return arregloElementos.filter((elemento,index, self) => index === self.indexOf(elemento));
}
